...
namespace Microsoft.Extensions.Caching.Memory
{
    ...
    public class MemoryCache : IMemoryCache
    {
        ...
        private bool _expirationScanInProgress;
        ...
        public bool TryGetValue(object key, out object result)
        {
            ...
            if (_entries.TryGetValue(key, out entry))
            {
                ...
                if (entry.CheckExpired(utcNow)  && entry.EvictionReason != EvictionReason.Replaced && 
                   (entry.DropOutToken != null) && entry.DropOutToken.DropOutRequest) 
                {
        ...
        { 
            ...
            if ((_expirationScanFrequency < now - _lastExpirationScan) && !_expirationScanInProgress)                  // _lastExpirationScan is poor condition if _expirationScanFrequency is too low value or time of scan is too long
            {
              _lastExpirationScan = now;
              _expirationScanInProgress = true;

              Task.Factory.StartNew(state => { ScanForExpiredItems((MemoryCache)state); _expirationScanInProgress = false;}, 
